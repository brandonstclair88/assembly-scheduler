...
export function WeeklyBoard({data,setData,schedule,warnings,projectHealthById,boardIntent,onOpenProject}:any){
 ...
 function isFri(date:string){return new Date(date+'T00:00:00').getDay()===5}
	 function toggleTestFri(date:string){if(boardMode==='Live')return;setData((d:any)=>{const vals=splitIds((d as any).testFridayDates||'');const next=vals.includes(date)?vals.filter((x:string)=>x!==date):[...vals,date];return {...d,testFridayDates:next.join(',')}})}
	 function testFridayOn(date:string){return splitIds((data as any).testFridayDates||'').includes(date)}
 ...
 function testItemsFor(date:string){
   ...
   for(const asm of (data.projectAssemblies||[])){
     ...
     if(date>buildEnd&&date<=testEnd&&(!isFri(date)||testFridayOn(date)))rows.push(asm);
   }
...
function AssemblyDetailPanel(){
  ...
  return <div className="assemblyDetailPanel" style={{'--assembly-accent':assemblyAccentColor(asm.id)} as any}>
    ...
    <div className="assemblyDetailPhases">
      ...
      {insp&&<div className={'detailPhaseRow'+(insp.isLate?' late':'')}><span className="phaseBadge phase-finalize">FINALIZE</span><span>{fmtDate(insp.scheduledStart)} → {fmtDate(insp.scheduledEnd)}</span><span>{Number(insp.totalHours||0).toFixed(1)} hrs</span><span>{insp.assignedEmployeeNames||'Unassigned'}</span></div>}
