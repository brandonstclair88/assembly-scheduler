...
function changeAsm(id:string,patch:any){setData((d:any)=>applyAssemblyPatch(d,id,patch))}
 ...
 function TopLevelCard({top}:any){
  ...
  return <div className={`buildCard buildCardRedesign ${statusClass}`}>
    ...
    <CollapsibleSection storageKey={`top-${top.id}-advanced`} title="Advanced Scheduling" subtitle="Batching, sequencing, test return, and finalizing / shipping crews." defaultOpen={false} summary={`${batch?.name||'No batch'} • ${topSeq?'Sequenced':'No sequencing'} • ${top.lateAllowed?'Late allowed':'On-time required'}${top.testRequired?' • Test gate':''}`}>
    <div className="buildControls">
