...
export function Schedule({data,schedule}:any){
 return <div className="card"><h2>Master Schedule</h2><p className="muted">Read-only computed schedule. Edit assemblies from the Projects page or move work on the Weekly Board.</p><div className="tablewrap"><table><thead><tr>{['Week','Project','Phase','Part #','#','Description','Deps','Employees','Start','Finish / Ship','Hours','Test','Finalizing','Shipping','%','Status','Late','Assembly Ship By'].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{schedule.map((r:any)=><tr key={r.id} className={r.isLate?'late':''}><td>{fmtDate(r.week)}</td><td>{r.projectName}</td><td>{r.phase||'Build'}</td><td>{r.partNumber}</td><td>{r.instanceLabel||''}</td><td>{r.description}</td><td>{r.dependencyNames}</td><td>{r.assignedEmployeeNames}</td><td>{fmtDate(r.scheduledStart)}</td><td>{fmtDate(r.scheduledEnd)}</td><td>{r.totalHours} total / {r.hoursPerEmployee.toFixed(1)} ea</td><td>{r.testRequired?`${r.testHours||0} hrs`:''}</td><td>{r.finalizingRequired?`${r.finalizingHours||0} hrs`:''}</td><td>{r.shippingRequired?`${r.shippingHours||0} hrs`:''}</td><td>{r.phase==='Build'?`${r.percent||0}%`:(r.phase==='Finalizing'?(r.finalizingComplete?'Done':'Open'):(r.shippingComplete?'Done':'Open'))}</td><td>{r.status}</td><td>{r.isLate?'Yes':''}</td><td>{fmtDate(r.shipDate)}</td></tr>)}</tbody></table></div></div>
}
   ...
   {tops.map((top:any)=>{
    ...
    return <div className="flowRow" key={top.id}>
     ...
     <div className="flowRowChips">
      ...
      {itemFor(top.id,'Finalizing')&&<span className="flowArrow">→</span>}
      {chip(top,'Finalizing')}
