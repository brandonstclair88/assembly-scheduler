...
function WeeklyBoard({data,setData,schedule,warnings,projectHealthById,boardIntent,onOpenProject}:any){
 ...
 const chunkMeta=(()=>{
   ...
   Object.values(byKey).forEach((list:any[])=>{
     ...
     let run=0;
     sorted.forEach((c:any,i:number)=>{meta.set(c,{index:i+1,count:sorted.length,hrsBefore:run});run+=Number(c.chunkHours)||0});
   });
   return meta;
 })();
...
function TaskCard({s}:any){
  ...
  const m=chunkMeta.get(s)||{index:(Number(s.segmentIndex)||0)+1,count:1,hrsBefore:0};
  ...
  return <div className={'task phaseCard phase-'+phaseTone+' status'+status.replace(/\s+/g,'')+' '+asmFocus+(locked?'lockedTask ':'')+(buildTileDone?'buildTileDone ':'')+(buildTilePartial?'buildTilePartial ':'')+(s.chunkLabel==='Draft'?'draftTask ':'')+(s.isLive?' liveForecastTask ':'')+(s.forecastMoved?' forecastMovedTask ':'')+(s.forecastBlocked?' forecastBlockedTask ':'')+(dimmed?'taskDimmed ':'')+(projectFocusId!=='All'&&!dimmed?'taskFocused ':'')+(autoAssigned?' autoAssignedTask ':'')+((boardDensity==='compact'||boardDensity==='ultra')?'compactTask ':'')+(boardDensity==='ultra'?'ultraCompactTask ':'')} style={{'--project-accent':projectAccentColor(source.projectId||s.projectId||''),'--assembly-accent':assemblyAccentColor(src)} as any} draggable={!locked&&boardMode!=='Live'} onDragStart={boardMode==='Live'?undefined:startDrag} onDragEnd={stopAutoScroll} onClick={tileClick}><span className="asmStripe"/><div className="taskBadgeRow"><span className={`phaseBadge phase-${phaseTone}`}>{phaseLabel}</span><span className="statusBadge">{status}</span>{locked&&<span className="forecastBadge">LOCK</span>}{autoAssigned&&<span className="forecastBadge autoBadge">AUTO</span>}{s.isLive&&<span className="forecastBadge">Live</span>}{s.forecastMoved&&<span className="forecastMovedBadge">Moved</span>}{s.isLate&&<span className="lateBadge">late</span>}</div><div className="taskProgress"><div className="taskProgressFill" style={{width:`${Math.max(0,Math.min(100,pct))}%`}}/></div><b className="tileDescription">{(source.instanceLabel||s.instanceLabel)&&<span className="tileAssemblyNo">{source.instanceLabel||s.instanceLabel}</span>}{source.description||s.description}</b>{s.batchId&&<span className="batchBadge">{(data.shipmentBatches||[]).find((b:any)=>b.id===s.batchId)?.name}</span>}<span className="tileMeta">{tileProject?.projectId||s.projectName} · P/N {source.partNumber||s.partNumber}</span><span className="chunkHours">Day {m.index}/{m.count} · {Number(s.chunkHours).toFixed(1)} hrs{phase==='Build'&&totalHrs?` · ${m.hrsBefore.toFixed(1)}–${tileEnd.toFixed(1)} of ${totalHrs.toFixed(1)}`:''}</span>{!s.employeeChunkId&&suggestion?.employeeName&&<span className="suggestedAssignBadge">Suggest: {suggestion.employeeName}</span>}{suggestion?.nonPreferredButNecessary&&<span className="suggestedAssignBadge warn">Non-preferred but needed</span>}{s.chunkLabel==='Manual'&&<span className="splitBadge">manual</span>}{s.chunkLabel==='Partial'&&<span className="splitBadge">split</span>}{s.chunkLabel==='Live Hold'&&<span className="splitBadge">hold</span>}</div>
}
