...
const PROJECT_HEALTH_OPTIONS=['All','On Track','At Risk','Late','Missing Assignment','Over Capacity','Waiting on Test','Waiting on Finalizing','Ready to Ship'];
function phaseBadgeLabel(phase:string){return phase==='Finalizing'?'FINALIZING':phase==='Shipping'?'SHIP':phase==='Test'?'TEST':'BUILD'}
function phaseToneKey(phase:string){return phase==='Finalizing'?'finalize':phase==='Shipping'?'ship':phase==='Test'?'test':'build'}
...
function PageNavDropdown({tab,setTab,primaryNav,secondaryNav}:any){
 ...
 const current=all.find((item:any)=>item.tab===tab);
 function choose(t:string){setTab(t);setOpen(false)}
 return <div className="pageNav" ref={ref}>
  <button type="button" className="pageNavTrigger" aria-haspopup="true" aria-expanded={open} onClick={()=>setOpen(v=>!v)}>
   <span className="pageNavCurrentLabel">Page</span>
   <span className="pageNavCurrent">{current?current.label:'Menu'}</span>
   <span className={`pageNavChevron${open?' open':''}`} aria-hidden="true">▾</span>
  </button>
  {open&&<div className="pageNavMenu" role="menu">
   <div className="pageNavGroup">
    <div className="pageNavGroupLabel">Pages</div>
    {primaryNav.map((item:any)=><button key={item.tab} type="button" role="menuitem" className={`pageNavItem${tab===item.tab?' active':''}`} onClick={()=>choose(item.tab)}>{item.label}</button>)}
   </div>
   <div className="pageNavGroup">
    <div className="pageNavGroupLabel">Planning Tools</div>
    {secondaryNav.map((item:any)=><button key={item.tab} type="button" role="menuitem" className={`pageNavItem${tab===item.tab?' active':''}`} onClick={()=>choose(item.tab)}>{item.label}</button>)}
   </div>
  </div>}
 </div>
}
...
function Planner({data,setData,schedule,warnings,projectHealth,setTab}:any){
 ...
 function roleAllowed(emp:any,phase:string){if(phase==='Finalizing')return emp.canFinalize!==false;if(phase==='Shipping')return emp.canShip!==false;return emp.canBuild!==false}
 ...
 for(const date of Array.from({length:horizonDaysMap[plannerHorizon]+1},(_,i)=>addDays(today,i))){
   ...
   if(!weeklyRoleLoad[week])weeklyRoleLoad[week]={week,buildScheduled:0,buildAvailable:0,finalizingScheduled:0,finalizingAvailable:0,shippingScheduled:0,shippingAvailable:0,overloads:0};
   for(const employee of employees){
     ...
     if(roleAllowed(employee,'Finalizing'))weeklyRoleLoad[week].finalizingAvailable+=cap;
 ...
 for(const c of chunks){
   ...
   if(phase==='Finalizing')weeklyRoleLoad[week].finalizingScheduled+=Number(c.chunkHours)||0;
 ...
 const weeklyCapacityRows=Object.values(weeklyRoleLoad).sort((a:any,b:any)=>String(a.week).localeCompare(String(b.week))).map((row:any)=>({...row,shipments:weekShipCounts[row.week]||0,buildPct:row.buildAvailable?row.buildScheduled/row.buildAvailable:0,finalizingPct:row.finalizingAvailable?row.finalizingScheduled/row.finalizingAvailable:0,shippingPct:row.shippingAvailable?row.shippingScheduled/row.shippingAvailable:0}));
 const roleSummary=[['Build','buildScheduled','buildAvailable'],['Finalizing','finalizingScheduled','finalizingAvailable'],['Shipping','shippingScheduled','shippingAvailable']].map(([label,scheduledKey,availableKey])=>{const scheduled=weeklyCapacityRows.reduce((sum:number,row:any)=>sum+Number(row[scheduledKey]||0),0);const available=weeklyCapacityRows.reduce((sum:number,row:any)=>sum+Number(row[availableKey]||0),0);const pct=available?scheduled/available:0;return {label,scheduled,available,pct};});
 ...
 for(const s of schedule){
   ...
   if(s.isLate)issues.push({type:'Late',severity:'red',assemblyId:a.id,phase:s.phase||'Build',date:s.scheduledEnd,title:`Late: ${taskLabel(a)}`,why:[`Scheduled to finish ${fmtDate(s.scheduledEnd)}.`,`Ship By is ${fmtDate(a.shipDate)||'not set'}.`,a.lateAllowed?'Late Allowed is checked.':'Late Allowed is not checked.'],suggestions:['Move earlier work into open capacity before the ship date.','Add another employee to the build/finalizing/shipping task.','If this is acceptable, check Late Allowed.']});
 }
 for(const a of assemblies){
   ...
   if(a.type==='Top Level Assembly'){
     ...
     const build=getItem(a.id,'Build'), finalizing=getItem(a.id,'Finalizing');
     if(build&&finalizing&&a.testRequired&&Number(a.testHours||0)>0){
       ...
       if(finalizing.scheduledStart<minReturn)issues.push({type:'Test Gate',severity:'yellow',assemblyId:a.id,phase:'Finalizing',date:finalizing.scheduledStart,title:`Finalizing starts before test gate: ${taskLabel(a)}`,why:[`Build finishes ${fmtDate(build.scheduledEnd)}.`,`Test requires ${a.testHours} hours${a.testReturnDateTime?` and expected return is ${fmtDateTime(a.testReturnDateTime)}`:''}.`,`Finalizing starts ${fmtDate(finalizing.scheduledStart)}.`],suggestions:['Move finalizing after the expected test return.','Enter/update Expected Test Return if test will take longer.']});
     }
   }
 }
 ...
 const overdueFinalizings=(warnings||[]).filter((warning:any)=>warning.code==='missing_finalizing_assignment'||warning.code==='sub_after_parent').slice(0,8);
 const blockedProjects=riskProjects.filter((record:any)=>record.status==='Waiting on Test'||record.status==='Waiting on Finalizing'||record.reason?.toLowerCase().includes('warning')).slice(0,8);
 ...
 return <div className="grid">
   ...
   {plannerView==='capacity'&&<>
     ...
     <div className="card span12"><h2>Planner Heatmap</h2><div className="plannerHeatmap">{weeklyCapacityRows.map((row:any)=><div key={row.week} className={`heatCell tone-${toneForPct(Math.max(row.buildPct,row.finalizingPct,row.shippingPct))}`}><b>{fmtDate(row.week)}</b><span>Build {Math.round(row.buildPct*100)}%</span><span>Finalize {Math.round(row.finalizingPct*100)}%</span><span>Ship {Math.round(row.shippingPct*100)}%</span><small>{row.overloads} overloaded cell{row.overloads===1?'':'s'} • {row.shipments} shipments</small></div>)}</div></div>
   </>}
   {plannerView==='risk'&&<>
   ...
   {plannerView==='shipments'&&<>
     <div className="card span12"><h2>Shipment Forecast</h2><div className="plannerHeatmap">{weeklyCapacityRows.map((row:any)=><div key={row.week} className={`heatCell tone-${row.shipments>=8?'critical':row.shipments>=4?'capacity':'info'}`}><b>{fmtDate(row.week)}</b><span>{row.shipments} shipment due{row.shipments===1?'':'s'}</span><span>Shipping load {Math.round(row.shippingPct*100)}%</span><small>Build {Math.round(row.buildPct*100)}% • Finalize {Math.round(row.finalizingPct*100)}%</small></div>)}</div></div>
...
function Projects({data,setData,schedule,warnings,projectHealth,projectHealthById,panelIntent,onFocusBoard}:any){
 ...
 const qualifiedFinalizers=project?smartAssignQualifiedEmployees(data,project.id,'Finalizing'):[];
 ...
 const preferredFinalizers=project?qualifiedFinalizers.filter((employee:any)=>employeePrefersProject(employee,project.id)):[];
 ...
 const otherFinalizers=project?qualifiedFinalizers.filter((employee:any)=>!employeePrefersProject(employee,project.id)):[];
 ...
 function TopLevelCard({top}:any){
  ...
  return <div className={`buildCard buildCardRedesign ${statusClass}`}>
    ...
    <div className="buildControls">
      ...
      {top.finalizingRequired&&<div className="field"><label>Finalizing Employee(s)</label><EmployeePicker data={data} value={top.finalizingAssignedTo||''} onChange={(v:any)=>safeChange(top,{finalizingAssignedTo:v})} row={top} phase="Finalizing"/><label className="checkLine"><input type="checkbox" checked={!!top.finalizingComplete} onChange={e=>safeChange(top,{finalizingComplete:e.target.checked})}/> Finalizing complete</label></div>}
     ...
     <CollapsibleSection storageKey={`${sectionBase}-qualified`} title="Preferred / Available Employees" subtitle="Preferred people first, then other active people who fit the role and current availability rules." defaultOpen={false} summary={qualifiedSummary}>
       <section className="qualifiedEmployeesPanel">
         <div className="qualifiedEmployeeGrid">
           ...
           <div className="qualifiedEmployeeCol"><h3>Preferred Finalizers</h3>{preferredFinalizers.length===0?<p className="muted">No preferred finalizers right now.</p>:preferredFinalizers.map((employee:any)=><span key={employee.id} className="qualifiedEmployeeChip preferredEmployeeChip">{employee.name}</span>)}
             <h4>Other Available Finalizers</h4>{otherFinalizers.length===0?<p className="muted small">No other finalizers available.</p>:otherFinalizers.map((employee:any)=><span key={employee.id} className="qualifiedEmployeeChip">{employee.name}</span>)}</div>
...
function AssemblyLibrary({data,setData}:any){
 const blank=(type='Sub Assembly')=>({id:uid('tpl'),partNumber:'',description:'',type,defaultQty:1,hoursEach:1,testRequired:false,testHours:0,finalizingRequired:false,finalizingHours:0,shippingRequired:false,shippingHours:0,testReturnDateTime:'',finalizingAssignedTo:'',shippingAssignedTo:'',finalizingManualStartDate:'',shippingManualStartDate:'',finalizingComplete:false,shippingComplete:false,maxTopPercentWhenSubHeld:80,defaultDependsOn:'',notes:'',archived:false});
...
function AssemblyEditor({item,rows,data,changeById,patchItem,duplicate,archive,del,whereUsed}:any){
 ...
 return <div className="assemblyEditor"><div className="editorTitle"><div><h2>{item.partNumber||'New Assembly'}</h2><p className="muted">{item.type}{item.archived?' • Archived':''}</p></div><div className="actions"><button className="btn" onClick={()=>duplicate(item.id)}>Duplicate</button><button className="btn" onClick={()=>whereUsed(item.id)}>Where Used</button><button className="btn" onClick={()=>archive(item.id)}>{item.archived?'Unarchive':'Archive'}</button><button className="btn danger" onClick={()=>del(item.id)}>Delete</button></div></div>
   ...
   <section><h3>Schedule Requirements</h3><div className="reqGrid">{req('Test Required','testRequired','testHours')}{req('Finalizing Required','finalizingRequired','finalizingHours')}{req('Shipping Required','shippingRequired','shippingHours')}</div></section>
...
function Schedule({data,setData,schedule}:any){function changeAsm(id:string,patch:Partial<ProjectAssembly>){const source=String(id).split('|')[0];setData((d:any)=>({...d,projectAssemblies:d.projectAssemblies.map((a:any)=>a.id===source?{...a,...patch}:a)}))}return <div className="card"><h2>Master Schedule</h2><p className="muted">Project assemblies feed this schedule. Library items do not appear until added to a project.</p><div className="tablewrap"><table><thead><tr>{['Week','Project','Phase','Part #','#','Description','Deps','Employees','Start','Finish / Ship','Hours','Test','Finalizing','Shipping','%','Status','Late','Assembly Ship By','Actions'].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{schedule.map((r:any)=><tr key={r.id} className={r.isLate?'late':''}><td>{fmtDate(r.week)}</td><td>{r.projectName}</td><td>{r.phase||'Build'}</td><td>{r.partNumber}</td><td>{r.instanceLabel||''}</td><td>{r.description}</td><td>{r.dependencyNames}</td><td>{r.assignedEmployeeNames}</td><td>{fmtDate(r.scheduledStart)}</td><td>{fmtDate(r.scheduledEnd)}</td><td>{r.totalHours} total / {r.hoursPerEmployee.toFixed(1)} ea</td><td>{r.testRequired?`${r.testHours||0} hrs`:''}</td><td>{r.finalizingRequired?`${r.finalizingHours||0} hrs`:''}</td><td>{r.shippingRequired?`${r.shippingHours||0} hrs`:''}</td><td>{r.phase==='Build'?<><input className="tiny" type="number" value={r.percent||0} onChange={e=>changeAsm(r.id,{percent:Number(e.target.value)})}/><div className="muted">Overall {rolledCompletion(data,r)}%</div></>:<label className="checkLine"><input type="checkbox" checked={r.phase==='Finalizing'?!!r.finalizingComplete:!!r.shippingComplete} onChange={e=>changeAsm(r.id,r.phase==='Finalizing'?{finalizingComplete:e.target.checked}:{shippingComplete:e.target.checked})}/> Done</label>}</td><td><select value={r.status} onChange={e=>changeAsm(r.id,{status:e.target.value})}><option>Not Started</option><option>In Progress</option><option>Complete</option><option>On Hold</option></select></td><td>{r.isLate?'Yes':''}</td><td><StableDateInput type="date" value={r.shipDate||''} onCommit={(value:any)=>changeAsm(r.id,{shipDate:value})}/></td><td><button className="mini" onClick={()=>changeAsm(r.id,r.phase==='Finalizing'?{finalizingComplete:true}:r.phase==='Shipping'?{shippingComplete:true}:{percent:100,status:'Complete'})}>Complete</button><button className="mini" onClick={()=>changeAsm(r.id,{shipDate:''})}>Clear</button></td></tr>)}</tbody></table></div></div>}
function WeeklyBoard({data,setData,schedule,warnings,projectHealthById,boardIntent,onOpenProject}:any){
 ...
 const [hideComplete,setHideComplete]=useState(false);
	 ...
	 const [lastSlippageRun,setLastSlippageRun]=useState<any|null>(null);
	 const [showSlippageResults,setShowSlippageResults]=useState(false);
	 ...
	 const slippageSuggestions=useMemo(()=>previewScheduleSlippage(data,schedule,todayStr),[data,schedule,todayStr]);
	 useEffect(()=>{
	   if(!showSlippagePreview)return;
	   setSlippageSelection(slippageSuggestions.map((s:any)=>s.id));
	 },[showSlippagePreview,slippageSuggestions]);
	 function applyScheduleSlippage(selectionIds:string[]=slippageSelection){
	   ...
	   setLastSlippageRun(result);
	   setShowSlippageResults(true);
	 ...
	 const slippageResultsRef=useRef<HTMLDivElement>(null);
	 useEffect(()=>{
	   if(!lastSlippageRun)return;
	   const delays=[0,50,150,300,600];
	   const timers=delays.map(ms=>setTimeout(()=>{
	     slippageResultsRef.current?.scrollIntoView({behavior:'smooth',block:'start'});
	   },ms));
	   return()=>{timers.forEach(clearTimeout)};
	 },[lastSlippageRun]);
 ...
 function canDrop(id:string,employeeId:string,date:string){
   ...
   if((parsed.phase||'Build')==='Finalizing'){
     ...
     if(release&&date<release){
       ...
       alert(`Finalizing cannot start on ${fmtDate(date)}. This assembly is gated by test${testH?` (${testH} hrs)`:''} until ${fmtDate(release)}.`);
       return false;
     }
   }
   if((parsed.phase||'Build')==='Shipping'){
     ...
     if(shipRelease&&date<shipRelease){alert(`Shipping cannot start before upstream test/finalizing is available on ${fmtDate(shipRelease)}.`);return false;}
   }
 ...
 function visibleEmployeesForWeek(week:string){return boardEmployees.filter((emp:any)=>!collapseEmptyRows||employeeFilter!=='All'||weekHasEmployeeWork(week,emp.id))}
   ...
   setData((d:any)=>({...d,projectAssemblies:d.projectAssemblies.map((a:any)=>{
     ...
     if(phase==='Finalizing')return {...a,finalizingComplete:pct>=100};
...
function TaskCard({s}:any){
 ...
 function SlippagePreviewPanel(){
  return (
    <div className="autoAssignPreviewPanel">
      <div className="autoAssignPreviewHeader">
        <div>
          <h3>Behind Schedule Preview</h3>
          <p className="muted">Compares today's date against each Build %. Nothing is saved until you apply.</p>
        </div>
        <div className="scheduleWarningCounts">
          <span className="warningCount critical">{slippageSuggestions.length} behind</span>
        </div>
      </div>
      <div className="smartAssignSelectionBar">
        <span className="muted">{slippageSelection.filter((id:string)=>slippageSuggestions.some((s:any)=>s.id===id)).length} of {slippageSuggestions.length} selected.</span>
        <div className="actions">
          <button className="btn" onClick={()=>setSlippageSelection(slippageSuggestions.map((s:any)=>s.id))}>Select all</button>
          <button className="btn" onClick={()=>setSlippageSelection([])}>Clear selection</button>
        </div>
      </div>
      <div className="autoAssignCompactList">
        {slippageSuggestions.length===0&&<p className="muted">Nothing is behind schedule right now - every in-progress Build is at or ahead of where today's date says it should be.</p>}
        {slippageSuggestions.map((suggestion:any)=>{
          const tone=suggestion.willBeLate?'critical':'capacity';
          const selected=slippageSelection.includes(suggestion.id);
          const expanded=expandedSuggestionIds.includes(suggestion.id);
          return (
            <div key={suggestion.id} className={`autoAssignRow ${tone}${expanded?' expanded':''}`}>
              <div className="autoAssignRowMain" onClick={()=>toggleSuggestionExpanded(suggestion.id)}>
                <input type="checkbox" checked={selected} onClick={e=>e.stopPropagation()} onChange={e=>setSlippageSelection((value:string[])=>e.target.checked?[...value,suggestion.id]:value.filter(id=>id!==suggestion.id))}/>
                <span className={`warningLevel ${tone}`}>BUILD</span>
                <b>{suggestion.projectCode}</b>
                <span className="autoAssignRowDesc">{suggestion.partNumber} — {suggestion.description}</span>
                <small>{suggestion.employeeNames}</small>
                <small>{Math.round(suggestion.actualPercent)}% done, expected {Math.round(suggestion.expectedPercent)}%</small>
                {suggestion.willBeLate&&<small>Past ship date</small>}
                <span className={`autoAssignRowChevron${expanded?' open':''}`}>▾</span>
              </div>
              {expanded&&<div className="autoAssignRowDetail">
                <div className="smartAssignReasonMeta">
                  <small>Was due {fmtDate(suggestion.originalBuildEnd)}</small>
                  <small>New build finish {fmtDate(suggestion.newBuildEnd)}</small>
                  {suggestion.newFinalizingStart&&<small>Finalizing now starts {fmtDate(suggestion.newFinalizingStart)}</small>}
                  {suggestion.newShippingStart&&<small>Shipping now starts {fmtDate(suggestion.newShippingStart)}</small>}
                  {suggestion.shipDate&&<small>Ship By {fmtDate(suggestion.shipDate)}</small>}
                </div>
                <small>{suggestion.reason}</small>
              </div>}
      ...
      <div className="warningActionRow">
        <button className="btn primary" disabled={!slippageSelection.length||boardMode==='Live'} onClick={()=>applyScheduleSlippage()}>Apply Selected {slippageSelection.length?`(${slippageSelection.length})`:''}</button>
        <button className="btn" disabled={!slippageSuggestions.length||boardMode==='Live'} onClick={()=>applyScheduleSlippage(slippageSuggestions.map((s:any)=>s.id))}>Apply All</button>
        <button className="btn" onClick={()=>setShowSlippagePreview(false)}>Cancel</button>
      </div>
    </div>
  );
 }
 function SlippageResultsPanel(){
  const items=[...(lastSlippageRun.applied||[]),...(lastSlippageRun.skipped||[]),...(lastSlippageRun.failed||[])];
  return (
    <div className="autoAssignResultsPanel" ref={slippageResultsRef}>
      <div className="autoAssignPreviewHeader">
        <div>
          <h3>Last Behind-Schedule Push</h3>
          <p className="muted">Applied pushes use the same save path as manual drag moves, so the board, Projects, and Dashboard stay in sync.</p>
        </div>
        <div className="scheduleWarningCounts">
          <span className="warningCount info">{lastSlippageRun.applied.length} applied</span>
          <span className="warningCount capacity">{lastSlippageRun.skipped.length} skipped</span>
          <span className="warningCount critical">{lastSlippageRun.failed.length} failed</span>
        </div>
      </div>
      <div className="warningActionRow">
        <button className="btn" onClick={()=>setShowSlippageResults((value:any)=>!value)}>{showSlippageResults?'Hide Results':'Show Results'}</button>
        <button className="btn" onClick={()=>setLastSlippageRun(null)}>Clear</button>
      </div>
      {showSlippageResults&&<div className="autoAssignCompactList">{items.length===0?<p className="muted">Nothing was applied this run.</p>:items.map((suggestion:any)=>{const tone=suggestion.applyStatus==='applied'?'info':suggestion.applyStatus==='skipped'?'capacity':'critical';const rowKey=`slip-result-${suggestion.id}-${suggestion.applyStatus}`;const expanded=expandedSuggestionIds.includes(rowKey);return <div key={rowKey} className={`autoAssignRow ${tone}${expanded?' expanded':''}`}><div className="autoAssignRowMain" onClick={()=>toggleSuggestionExpanded(rowKey)}><span className={`warningLevel ${tone}`}>BUILD</span><b>{suggestion.projectCode}</b><span className="autoAssignRowDesc">{suggestion.partNumber} — {suggestion.description}</span><small>{suggestion.applyStatus==='applied'?'Applied':suggestion.applyStatus==='skipped'?'Skipped':'Failed'}</small><span className={`autoAssignRowChevron${expanded?' open':''}`}>▾</span></div>{expanded&&<div className="autoAssignRowDetail"><div className="smartAssignReasonMeta"><small>New build finish {fmtDate(suggestion.newBuildEnd)}</small>{suggestion.shipDate&&<small>Ship By {fmtDate(suggestion.shipDate)}</small>}</div><small>{suggestion.applyReason}</small><small className="muted">{suggestion.reason}</small></div>}</div>})}</div>}
    </div>
  );
 }
 return <div className={`card weeklyBoardCard density-${boardDensity}`}><div className="boardHeader"><div><h2>Weekly Board</h2><p className="muted">{boardMode==='Live'?'Read-only forecast from saved schedule, status, holds, time off, tests, dependencies, and ship rules.':'Drag individual daily chunks. Moves stay in draft mode until you click Apply Changes, so accidental moves can be discarded.'}</p></div><div className="boardTools enhancedBoardTools">
  ...
  <div className="boardToolGroup boardToolGroupActions">
   ...
   <div className="boardToolGroupRow">
    ...
    <button className="btn" title="Compares today's date against each Build % and previews pushing behind-schedule work forward into open capacity. Nothing is saved until you apply." disabled={boardMode==='Live'} onClick={()=>setShowSlippagePreview(true)}>Check Behind Schedule{slippageSuggestions.length>0&&<span className="buttonBadge warn">{slippageSuggestions.length} Behind</span>}</button>
   </div>
  </div>
