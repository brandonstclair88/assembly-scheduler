...
import { buildSchedule, capacityForDate, scheduleHealth } from '../lib/scheduler';
...
export default function MobileViewer() {
  ...
  function testItemsForDay(date: string) {
    const rows: any[] = [];
    // Match the desktop board: the test wait uses the shared calendar (which now
    // honors per-Friday Test OT via testFridayDates), and a test only shows on a
    // Friday column when that Friday is enabled as a test day.
    const isFriday = new Date(`${date}T00:00:00`).getDay() === 5;
    const testFridayEnabled = splitIds(String((data as any).testFridayDates || '')).includes(date);
    for (const assembly of (data.projectAssemblies || [])) {
      ...
      const estimated = Number(assembly.testHours || 0) > 0 ? externalWaitEnd(data, buildEnd, Number(assembly.testHours || 0)) : buildEnd;
      ...
      if (date > buildEnd && date <= testEnd && (!isFriday || testFridayEnabled)) rows.push(assembly);
    }
