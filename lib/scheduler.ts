...
function employeeCan(e:any,phase:'Build'|'Inspection'|'Shipping'){if(!e||e.active===false)return false;if(phase==='Inspection')return e.canInspect!==false;if(phase==='Shipping')return e.canShip!==false;return e.canBuild!==false}
...
function firstInternalWorkDateAfterTest(buildFinish:string,a:Assembly,data:AppData){
  ...
  // as consumed by test. Inspection/shipping should start on the next available
...
function latestBuildFinishBeforeTestDownstream(downstreamStart:string,a:Assembly,data:AppData){
  ...
  // to make that return realistic. Inspection/shipping are still held until that date.
  if(manualReturn){
...
export function buildSchedule(data:AppData):ScheduledItem[]{
  ...
  function assigneesFor(a:Assembly,phase:'Build'|'Inspection'|'Shipping'){
    const raw=phase==='Inspection'?(a.inspectionAssignedTo||a.assignedTo):phase==='Shipping'?(a.shippingAssignedTo||a.assignedTo):a.assignedTo;
  ...
  function makeItem(a:Assembly,phase:'Build'|'Inspection'|'Shipping',start:string,finish:string,totalHours:number,hoursPerEmployee:number,assignees:string[],deps:string[],isLate:boolean):ScheduledItem{
    const proj=projects[a.projectId];
    const phasePrefix=phase==='Build'?'BUILD':phase==='Inspection'?'INSPECT':'SHIP';
  ...
  function phasePlan(a:Assembly,phase:'Inspection'|'Shipping',deadline:string){
    const hours=phase==='Inspection'?(Number(a.inspectionHours)||0):(Number(a.shippingHours)||0);
  ...
  function addPhase(a:Assembly,phase:'Inspection'|'Shipping',targetStart:string,targetEnd:string,afterDate:string,shipDate:string){
    const hours=phase==='Inspection'?(Number(a.inspectionHours)||0):(Number(a.shippingHours)||0);
    if(hours<=0)return {start:afterDate,end:afterDate,item:null as any,late:false};
    const ids=assigneesFor(a,phase); const count=Math.max(1,ids.length); const hpe=hours/count;
    const manual=phase==='Inspection'?a.inspectionManualStartDate:a.shippingManualStartDate;
    ...
    const item=makeItem(a,phase,start,end,hours,hpe,ids,[phase==='Inspection'?'Build/Test complete':'Inspection complete'],late);
  ...
  function scheduleOne(a:Assembly,requestedFinish?:string,seen=new Set<string>()):ScheduledItem{
    ...
    const inspHours=a.inspectionRequired?(Number(a.inspectionHours)||0):0;
    ...
    const inspectionDeadline=a.shippingRequired?previousWorkdayBefore(shipTarget.start,data,shipTarget.ids):shipDate;
    const inspectionTarget=a.inspectionRequired?phasePlan(a,'Inspection',inspectionDeadline):{start:inspectionDeadline,end:inspectionDeadline,hours:0,hpe:0,ids:[] as string[]};
    const downstreamStart=a.inspectionRequired?inspectionTarget.start:inspectionDeadline;
    ...
    let missedWindow=!!(readyForInternal&&inspectionTarget.start&&readyForInternal>inspectionTarget.start);
    if(a.inspectionRequired){const p=addPhase(a,'Inspection',inspectionTarget.start,inspectionTarget.end,readyForInternal,shipDate); if(p.item)scheduled[a.id+'|inspection']=p.item; gate=p.end||gate; finalCompletion=p.end||finalCompletion; missedWindow=missedWindow||p.late;}
    ...
    if(!a.inspectionRequired&&!a.shippingRequired)finalCompletion=testGateEnd(finish,a,data);
    const isLate=!!(shipDate&&(finalCompletion>shipDate||missedWindow));
    if(isLate){
      scheduled[a.id]={...scheduled[a.id],isLate:true};
      if(scheduled[a.id+'|inspection'])scheduled[a.id+'|inspection']={...scheduled[a.id+'|inspection'],isLate:true};
...
export function suggestEmployees(data:AppData,ignoreAssemblyId?:string,count=1,phase:'Build'|'Inspection'|'Shipping'='Build'){
